...
export function HomeAdvertisement({ sx, ...other }: StackProps) {
  ...
  const renderDescription = (
    <Stack spacing={5} sx={{ zIndex: 9, px: 28, mx: 'auto' }}>
      <SectionTitle
        caption="Get all news"
        title="Subscribe to"
        txtGradient="the newsletter"
        description={`Here you will find important tips & tricks for your daily fasting routine and new information on the topic of fasting`}
        sx={{ mb: 8, textAlign: 'center', color: 'white' }}
      />
      <TextField
        fullWidth
        ...
        InputProps={{
          endAdornment: (
            <InputAdornment position="end">
              <Button onClick={() => {}} sx={{ mr: -0.5, color: 'white' }}>
                Apply
              </Button>
            </InputAdornment>
          ),
      ...
      >
        No advertising, no tracking, no obligation
      </Typography>
    </Stack>
  );
