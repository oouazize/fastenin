import type { BoxProps } from '@mui/material/Box';
import type { StackProps } from '@mui/material/Stack';
import type { MotionProps, MotionValue } from 'framer-motion';

import { useRef, useState, forwardRef } from 'react';
import { m, useSpring, useScroll, useTransform, useMotionValueEvent } from 'framer-motion';

import Box from '@mui/material/Box';
import Stack from '@mui/material/Stack';
import Button from '@mui/material/Button';
import Container from '@mui/material/Container';
import Grid from '@mui/material/Unstable_Grid2';
import Typography from '@mui/material/Typography';
import { styled, useTheme } from '@mui/material/styles';

import { paths } from 'src/routes/paths';

import { useClientRect } from 'src/hooks/use-client-rect';

import { CONFIG } from 'src/config-global';

import { Iconify } from 'src/components/iconify';
import { varFade, MotionViewport } from 'src/components/animate';

import { SectionTitle, SectionCaption } from './components/section-title';
import { FloatLine, FloatTriangleLeftIcon } from './components/svg-elements';
import { SvgColor } from 'src/components/svg-color';
import { Image } from 'src/components/image';

// ----------------------------------------------------------------------

export function HomeHugePackElements({ sx, ...other }: StackProps) {
  const renderLines = (
    <>
      <FloatTriangleLeftIcon sx={{ top: 80, left: 80, opacity: 0.4 }} />
      <FloatLine vertical sx={{ top: 0, left: 80 }} />
    </>
  );

  return (
    <Stack component="section" sx={{ pt: 60, position: 'relative', ...sx }} {...other}>
      <MotionViewport>
        {renderLines}

        <Container sx={{ textAlign: { xs: 'center', md: 'left' } }}>
          <Grid
            container
            disableEqualOverflow
            rowSpacing={{ xs: 3, md: 0 }}
            columnSpacing={{ xs: 0, md: 8 }}
          >
            <Grid xs={12} md={6} lg={7}>
              <SectionTitle
                caption="Your way"
                title="Fasting hiking in "
                txtGradient="Germany, Austria and Switzerland"
                sx={{ mb: { xs: 5, md: 8 }, textAlign: { xs: 'center', md: 'left' } }}
              />
              <Stack
                spacing={4}
                sx={{
                  maxWidth: { sm: 560, md: 400 },
                  mx: { xs: 'auto', md: 'unset' },
                }}
              >
                {ITEMS.map((item) => (
                  <Box
                    component={m.div}
                    key={item.title}
                    variants={varFade({ distance: 24 }).inUp}
                    gap={3}
                    display="flex"
                  >
                    <SvgColor src={item.icon} sx={{ width: 40, height: 40 }} />
                    <Stack spacing={1}>
                      <Typography variant="h5" component="h6">
                        {item.title}
                      </Typography>
                      <Typography sx={{ color: 'text.secondary' }}>{item.description}</Typography>
                    </Stack>
                  </Box>
                ))}
              </Stack>
            </Grid>

            <Grid xs={12} md={6} lg={5}>
              <m.div variants={varFade({ distance: 24 }).inUp}>
                <Typography
                  sx={{ color: 'text.disabled', fontSize: { md: 20 }, lineHeight: { md: 36 / 20 } }}
                >
                  <Box component="span" sx={{ color: 'text.primary' }}>
                    Far away from the stress of everyday life, we offer you the chance to find new
                    energy and clarity through fasting. It is not just a sacrifice, but a gain for
                    your overall well-being. Try it out and experience the difference.
                  </Box>
                </Typography>
                <Box>
                  <Box sx={{ px: 2, mt: 2 }}>
                    <Box sx={{ position: 'relative', pl: 6, mb: 20 }}>
                      <Box
                        component="img"
                        src={`${CONFIG.site.basePath}/assets/images/home/facts-0.jpeg`}
                        sx={{
                          borderRadius: 1,
                          maxheight: '600px',
                          transform: { xs: 'scaleX(-1)' },
                        }}
                      />
                      <Box
                        component="img"
                        src={`${CONFIG.site.basePath}/assets/images/home/facts-1.jpeg`}
                        sx={{
                          top: '18%',
                          left: 0,
                          maxHeight: '500px',
                          position: 'absolute',
                          borderRadius: 1,
                        }}
                      />
                    </Box>
                  </Box>
                </Box>
              </m.div>
            </Grid>
          </Grid>

          <m.div variants={varFade({ distance: 24 }).inUp}>
            <Button
              size="large"
              color="inherit"
              variant="outlined"
              target="_blank"
              rel="noopener"
              href={paths.components}
              endIcon={<Iconify icon="eva:arrow-ios-forward-fill" />}
              sx={{ mt: 5, mx: 'auto' }}
            >
              Browse fasting hikes
            </Button>
          </m.div>
        </Container>
      </MotionViewport>

      {/* <ScrollContent /> */}
    </Stack>
  );
}

// ----------------------------------------------------------------------

const StyledRoot = styled(
  forwardRef((props: BoxProps & MotionProps, ref) => <Box ref={ref} component={m.div} {...props} />)
)(({ theme }) => ({
  zIndex: 9,
  position: 'relative',
  paddingTop: theme.spacing(5),
  [theme.breakpoints.up('md')]: { paddingTop: theme.spacing(15) },
}));

const StyledContainer = styled((props: MotionProps & Omit<BoxProps, 'style'>) => (
  <Box component={m.div} {...props} />
))(({ theme }) => ({
  top: 0,
  height: '100vh',
  display: 'flex',
  position: 'sticky',
  overflow: 'hidden',
  flexDirection: 'column',
  justifyContent: 'flex-start',
  transition: theme.transitions.create(['background-color']),
  '&[data-scrolling="true"]': { justifyContent: 'center' },
}));

const StyledContent = styled(
  forwardRef((props: BoxProps & MotionProps, ref) => (
    <Box ref={ref} component={m.div} transition={{ ease: 'linear', duration: 0.25 }} {...props} />
  ))
)(({ theme }) => ({
  display: 'flex',
  flexDirection: 'column',
  gap: theme.spacing(3),
  [theme.breakpoints.up('md')]: { gap: theme.spacing(5) },
}));

const StyledItem = styled((props: BoxProps & MotionProps) => <Box component={m.div} {...props} />)({
  backgroundSize: 'auto 100%',
  backgroundRepeat: 'repeat-x',
  backgroundPosition: 'center center',
});

// ----------------------------------------------------------------------

function ScrollContent() {
  const theme = useTheme();

  const containerRef = useRef<HTMLDivElement>(null);
  const containerRect = useClientRect(containerRef);

  const scrollRef = useRef<HTMLDivElement>(null);
  const scrollRect = useClientRect(scrollRef);

  const [startScroll, setStartScroll] = useState(false);

  const { scrollYProgress } = useScroll({ target: containerRef });

  const physics = { damping: 16, mass: 0.16, stiffness: 50 };

  const scrollRange = -scrollRect.scrollWidth + containerRect.width;

  const x1 = useSpring(useTransform(scrollYProgress, [0, 1], [0, scrollRange]), physics);
  const x2 = useSpring(useTransform(scrollYProgress, [0, 1], [scrollRange, 0]), physics);

  const background: MotionValue<string> = useTransform(
    scrollYProgress,
    [0, 0.25, 0.5, 0.75, 1],
    [
      theme.vars.palette.background.default,
      theme.vars.palette.background.neutral,
      theme.vars.palette.background.neutral,
      theme.vars.palette.background.neutral,
      theme.vars.palette.background.default,
    ]
  );

  useMotionValueEvent(scrollYProgress, 'change', (latest) => {
    if (latest !== 0 && latest !== 1) {
      setStartScroll(true);
    } else {
      setStartScroll(false);
    }
  });

  return (
    <StyledRoot ref={containerRef} sx={{ height: scrollRect.scrollWidth, minHeight: '100vh' }}>
      <StyledContainer style={{ background }} data-scrolling={startScroll}>
        <StyledContent ref={scrollRef} layout>
          <StyledItem
            style={{ x: x1 }}
            sx={{
              height: { xs: 160, md: 180 },
              width: { xs: '600%', md: '400%' },
              backgroundImage: `url(${CONFIG.site.basePath}/assets/images/home/bundle-${theme.palette.mode}-1.webp)`,
            }}
          />
          <StyledItem
            style={{ x: x2 }}
            sx={{
              height: { xs: 400, md: 480 },
              width: { xs: '600%', md: '400%' },
              backgroundImage: `url(${CONFIG.site.basePath}/assets/images/home/bundle-${theme.palette.mode}-2.webp)`,
            }}
          />
        </StyledContent>
      </StyledContainer>
    </StyledRoot>
  );
}

// ----------------------------------------------------------------------

const ITEMS = [
  {
    icon: `${CONFIG.site.basePath}/assets/icons/home/ic-filter.svg`,
    title: 'Purification and detoxification',
    description:
      'Fasting gives the digestive tract a break so the body can concentrate on detoxification.',
  },
  {
    icon: `${CONFIG.site.basePath}/assets/icons/home/ic-heart.svg`,
    title: 'Improved blood values',
    description: 'Fasting can help lower cholesterol and blood sugar levels.',
  },
  {
    icon: `${CONFIG.site.basePath}/assets/icons/home/ic-activity.svg`,
    title: 'Active lifestyle',
    description: 'Hiking is an excellent way to improve overall endurance and muscle strength.',
  },
  {
    icon: `${CONFIG.site.basePath}/assets/icons/home/ic-book.svg`,
    title: 'Mental clarity',
    description: 'The combination of fasting and hiking can help improve mental clarity and focus.',
  },
  {
    icon: `${CONFIG.site.basePath}/assets/icons/home/ic-users.svg`,
    title: 'Social networking',
    description:
      'The shared experience promotes social networking and can lead to long-lasting friendships.',
  },
];
